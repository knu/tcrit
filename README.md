...
- **Mouse-first TUI controls** — click file tabs, code lines, comment threads, sidebar items, dialog actions, and the review-finish button; use the wheel to scroll code and drag the gutter to select line ranges.  Click **☐ Resolve** / **☑︎ Resolved** in a thread header to toggle its resolution, including file comments.
